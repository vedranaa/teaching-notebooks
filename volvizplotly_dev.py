...
def volume_slicer(vol, slices, 
    ...
    ''' Visualizes chosen slices from volume.
        SUPPORT FOR RGB VOLUMES ADDED WITHOUT TESTING AND IS NOT DOCUMENTED AND 
        EXPERIMENTAL!!!
    ...
    dim = vol.shape[:3]
    RGB = True if (len(vol.shape) > 3) and (vol.shape[-1] == 3) else False
    ...
    surfaces = [go.Surface(s) for s in surfs]
